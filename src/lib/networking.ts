...
import { ADD_STEP_OF_BUILD, DELETE_BUILD, DELETE_STEP_IN_BUILD_STEPS, GET_ALL_PUBLICS_BUILDS, GET_ALL_STEPS_OF_BUILD_BY_BUILD_ID, GET_CONNECTED_USER_BUILDS, GET_CONNECTED_USER_ID, GET_PUBLIC_BUILD_BY_ID, MOVE_STEP_IN_BUILD_STEPS, PATCH_BUILD, PUBLISH_CONNECTED_USER_BUILD, SIGNIN, SIGNUP } from '../constants/api'
...
export const get_all_step_build_by_build_id = async (build_id: number) => {
    try {
        return await base_query_axios(GET_ALL_STEPS_OF_BUILD_BY_BUILD_ID, null, false, `/${build_id}`);
    } catch (error) {
        console.log(JSON.stringify(error));
    }
};
...
export const patch_build = async (build_id: number, build_metadata: any) => {
    ...
    try {
        const user_id = await get_connected_user_id();
        return await base_query_axios(PATCH_BUILD, { ...{ title: "" + title, description: "" + description, race: "" + race, v_race: "" + v_race, is_public: "" + is_public }, user_id: "" + user_id }, true, `/${build_id}`);
    } catch (error) {
        console.log(JSON.stringify(error));
    }
};
...
export const get_all_publics_builds = async () => {
    try {
        return await base_query_axios(GET_ALL_PUBLICS_BUILDS, null, false, `/all`);
    } catch (error) {
        console.log(JSON.stringify(error));
    }
};
...
export const get_public_build_by_id = async (id: number) => {
    try {
        return await base_query_axios(GET_PUBLIC_BUILD_BY_ID, null, false, `/${id}`);
    } catch (error) {
        console.log(JSON.stringify(error));
    }
};